...
<script>
import { UploadFiles, DeleteFiles } from "../../api/Safety/upLoad";
...
export default {
  ...
  computed: {
    ...
    // uploadUrlData: {
    // get() {
    // console.log("0000000000000000000000000000000000000",this.Safety.uploadUrlData)
    // let uploadUrlData = this.Safety.uploadUrlData
    // .filter((item) => {
  ...
  methods: {
    upLoad() {
      ...
      UploadFiles(this.formData).then((res) => {
        ...
        this.fileList[len]["source"] = "农产品质量安全检测能力建设";
    ...
    beforeUpload(file) {
      ...
      formData.append("task_id", this.Safety.userTaskId);
      formData.append("material_type", this.isFile);
      formData.append("source", "农产品质量安全检测能力建设");
