...
export default {
  ...
  methods: {
    ...
    handleAddHtml(val) {
      ...
      this.Modern.ModernData.modernInvestMent.push(str);
    },
    handleDeleteHtml(val) {
      let result = this.Modern.ModernData.modernInvestMent.filter((item) => {
        return item.inv_type == val;
      });
      let len = this.Modern.ModernData.modernInvestMent.lastIndexOf(
  ...
  watch: {
    data: {
      handler(val) {
        ...
        //   this.Modern.ModernData.modernInvestMent[1].inv_type == this.title;
        // }
