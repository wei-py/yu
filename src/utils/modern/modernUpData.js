...
export function modernData(newData) {
  ...
  for (let item in newData) {
    ...
    if (item == "modernFinances" && newData[item].length == 0) {
      //console.log('2222')
      newData[item] = ModernData[item];
    }

    if (item == "modernCauses" && newData[item].length == 0) {
      //console.log('4444')
      newData[item] = ModernData[item];
    ...
    if (item == "modernEquipment") {
      ...
      newData["modernInvestTotal"]["project_invest"] =
        newData[item][0].project_invest;
      newData["modernInvestTotal"]["support"] = newData[item][0].support;

      //console.log(newData['modernInvestTotal'])
    }
  }
